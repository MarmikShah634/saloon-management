...
let isRefreshing = false, queue: Array<(t: string) => void> = []
async function refreshToken(): Promise<string> {
  ...
  setTokens(data.access_token, data.refresh_token); return data.access_token
}
async function request<T>(path: string, init: RequestInit = {}, retried = false): Promise<T> {
  const token = getToken()
  const headers: HeadersInit = { 'Content-Type': 'application/json', 'X-Request-Id': crypto.randomUUID(), ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(init.headers ?? {}) }
  const res = await fetch(`${API_BASE}${path}`, { ...init, headers })
  if (res.status === 401 && !retried) {
    if (isRefreshing) return new Promise((resolve, reject) => { queue.push(t => request<T>(path, { ...init, headers: { ...headers, Authorization: `Bearer ${t}` } }, true).then(resolve).catch(reject)) })
    isRefreshing = true
    try { const t = await refreshToken(); queue.forEach(cb => cb(t)); queue = []; isRefreshing = false; return request<T>(path, init, true) }
